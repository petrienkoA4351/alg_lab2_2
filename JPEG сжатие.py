...
Original file is located at
    https://colab.research.google.com/drive/1UCqX22wcEWCP0DYx26Dbldoz-8NUBeRs
"""
...
def decompress(data, block_size=8):
    ...
    Cb_rec = upsample_420(Cb_rec, Y_rec.shape)
    Cr_rec = upsample_420(Cr_rec, Y_rec.shape)
...
def plot_individual_graphs(img_name, var_results, output_dir='results'):
    ...
    plot_configs = [
        {'key': 'RGB', 'title': 'Цветное изображение', 'color': 'blue', 'filename': f'{img_name}_RGB_size_vs_quality.png'},
        {'key': 'Grayscale', 'title': 'Оттенки серого', 'color': 'green', 'filename': f'{img_name}_Grayscale_size_vs_quality.png'},
        {'key': 'BW_Dither', 'title': 'Черно-белое с дизерингом', 'color': 'orange', 'filename': f'{img_name}_BW_Dither_size_vs_quality.png'},
        {'key': 'BW_No_Dither', 'title': 'Черно-белое без дизеринга', 'color': 'red', 'filename': f'{img_name}_BW_No_Dither_size_vs_quality.png'}
    ]
...
def display_all_restored_images(output_dir='results'):
    ...
    for img_name, var_data in images_data.items():
        ...
        plt.suptitle(f'Результаты сжатия: {img_name}', fontsize=16, y=1.02)
        plt.tight_layout()
...
if __name__ == "__main__":
    ...
    print("  - Графики для каждого типа изображения:")
